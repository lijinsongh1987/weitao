...
class RBAC {
    ...
    static public function AccessDecision($appName=APP_NAME) {
        ...
        if(RBAC::checkAccess()) {
            //存在认证识别号，则进行进一步的访问决策
            $action_name = RBAC::getActionName();
    ...
    static public function getActionName()
    {
        ...
        //查询是否菜单表
        $row =   D("Node")->where("level=3 and UPPER(name) ='$action_name'")->find();

        //如果节点表不存在,查询资源表
        if (!$row) {
           $data = M("Source")->where(" UPPER(source_name) ='$action_name'")->find();
            if($data){
                $node_id = $data["node_id"];
                $nodeRow = M("Node")->where("id = $node_id")->find();
                $action_name = $nodeRow["name"];
            }
        }
        return $action_name;
    }
